...
import { CREATE_COURSE, CREATE_LECTURER_COURSE, DELETE_COURSE, GET_CANDIDATE, GET_CANDIDATES, GET_COURSE, GET_COURSES, GET_LECTURERCOURSES_BY_COURSEID, GET_LECTURERS, LOGIN, LOGOUT, UPDATE_CANDIDATE_AVAILABLE, UPDATE_CANDIDATE_BLOCKED, UPDATE_COURSE } from "./graphql";
...
export const candidateService = {
    ...
    updateCandidateAvailable: async (
        id: string,
        candidate: {
            available: boolean;
        }
        ...
        const { data } = await client.mutate({
            mutation: UPDATE_CANDIDATE_AVAILABLE,
            variables: { id, ...candidate },
            refetchQueries: [{ query: GET_CANDIDATES }],
        });
        return data.updateCandidateAvailable;
    },
}